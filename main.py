...
def main_menu_window():
    ...
    while True:
        ...
        credits_button = Button(image=pygame.image.load("Images/Rects/Map Rect.png"), pos=(640, 400),
...
def already_bought():
    SCREEN.fill((255, 0, 0))
    my_font = pygame.font.SysFont('Comic Sans MS', 100)
    font_render = my_font.render('Item Has Already ', False, (0, 0, 0))
    font_render2 = my_font.render(' Been Purchased', False, (0, 0, 0))
    SCREEN.blit(font_render, (200, 150))
    SCREEN.blit(font_render2, (200, 400))
    pygame.display.update()
    time.sleep(1.5)
...
def not_enough_money():
    ...
    font_render2 = my_font.render('Purchase A New Item', False, (0, 0, 0))
...
def random_map_normal():
    ...
    for i in range(RANDOM_NUMBER):
        ...
        random_number2 = randint(0, num_maps)
        map_list2.append(segment_names[random_number2])
        if random_number2 == 16:
            segment_names.remove("Map_Segments/old_segments/segment_17")
    map_list2.append(map_list[1])
    lines = []
        ...
        for p in platforms:
            if pygame.sprite.collide_rect(self, p):
                ...
                if isinstance(p, PlatLevelEnd):
                    cash(money=RANDOM_NUMBER*randint(20, 40))
                    level_music(level=0)
                    play_window()
